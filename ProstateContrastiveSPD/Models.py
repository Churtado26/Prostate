...
class MertashBiParametricNetwork_WithOutBire(nn.Module): #Es la de yesid + SPD Pero sin el BIRE
    def __init__(self, device, target_shape, sequence_embedding_features=64, mode='classifier'):
        super(MertashBiParametricNetwork_WithOutBire, self).__init__()
        self._target_shape_ = target_shape
        self._sequence_embedding_features_ = sequence_embedding_features
        self._mode_ = mode
        self.t2_conv_branch =  self.__conv3d_block__()
    ...
    def forward(self, x):
        ...
        spd_output = Log.forward(self, covariance)
...
class Contrastive_CB3(nn.Module): #Contrastivo sin SPD con los CB de JO
    ...
    def forward(self, x):
        ...
        fusion_embedding = fusion_embedding.view(fusion_embedding.shape[0], -1)
        #print(f'fusion_embedding despues de concatenar: {fusion_embedding.shape}') #([32, 192*6*4*4])
